...
loop_columns          <- data.frame(matrix(nrow = 1, ncol = 5))
colnames(loop_columns) <- c('Date', 'Tank_number', 'Treatment', 'Count', 'Percent_Survival') # names for comuns in the for loop
...
for(r in 1:nrow(df_raw))  { # for each row
  ...
  for (c in 2:ncol(df_raw)) { # at each date... call tank columns one by one
      ...
      count.dead     <- cumulative.sum[r,] # the cumulative count dead at loop.date row r, column c
      
      count.alive    <- 20 - count.dead # all tanks started with 20 individuals!
  ...
  loop_columns$Count                  <- count.alive
